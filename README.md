...
- `docker-compose.yml` -- local Elasticsearch, required both for the `elasticsearch_rrf` production default and for re-running the evaluation notebook.
...
3. Start Elasticsearch (required for the default retrieval strategy):
   ```
   docker compose up -d
   ```
